...
`);try{_=new Worker(URL.createObjectURL(new Blob([w])))}catch(I){return typeof console<"u"&&typeof console.warn=="function"&&console.warn("🎊 Could not load worker",I),null}E(_)}return _}}(),m={particleCount:50,angle:90,spread:45,startVelocity:45,decay:.9,gravity:1,drift:0,ticks:200,x:.5,y:.5,shapes:["square","circle"],zIndex:100,colors:["#26ccff","#a25afd","#ff5e7e","#88ff5a","#fcff42","#ffa62d","#ff36ff"],disableForReducedMotion:!1,scalar:1};function g(_,h){return h?h(_):_}function M(_){return _!=null}function b(_,h,y){return g(_&&M(_[h])?_[h]:m[h],y)}function se(_){return _<0?0:Math.floor(_)}function re(_,h){return Math.floor(Math.random()*(h-_))+_}function oe(_){return parseInt(_,16)}function tt(_){return _.map(_o)}function _o(_){var h=String(_).replace(/[^0-9a-f]/gi,"");return h.length<6&&(h=h[0]+h[0]+h[1]+h[1]+h[2]+h[2]),{r:oe(h.substring(0,2)),g:oe(h.substring(2,4)),b:oe(h.substring(4,6))}}function po(_){var h=b(_,"origin",Object);return h.x=b(h,"x",Number),h.y=b(h,"y",Number),h}function mo(_){_.width=document.documentElement.clientWidth,_.height=document.documentElement.clientHeight}function go(_){var h=_.getBoundingClientRect();_.width=h.width,_.height=h.height}function yo(_){var h=document.createElement("canvas");return h.style.position="fixed",h.style.top="0px",h.style.left="0px",h.style.pointerEvents="none",h.style.zIndex=_,h}function vo(_,h,y,E,w,I,R,C,A){_.save(),_.translate(h,y),_.rotate(I),_.scale(E,w),_.arc(0,0,1,R,C,A),_.restore()}function Co(_){var h=_.angle*(Math.PI/180),y=_.spread*(Math.PI/180);return{x:_.x,y:_.y,wobble:Math.random()*10,wobbleSpeed:Math.min(.11,Math.random()*.1+.05),velocity:_.startVelocity*.5+Math.random()*_.startVelocity,angle2D:-h+(.5*y-Math.random()*y),tiltAngle:(Math.random()*(.75-.25)+.25)*Math.PI,color:_.color,shape:_.shape,tick:0,totalTicks:_.ticks,decay:_.decay,drift:_.drift,random:Math.random()+2,tiltSin:0,tiltCos:0,wobbleX:0,wobbleY:0,gravity:_.gravity*3,ovalScalar:.6,scalar:_.scalar,flat:_.flat}}function Eo(_,h){h.x+=Math.cos(h.angle2D)*h.velocity+h.drift,h.y+=Math.sin(h.angle2D)*h.velocity+h.gravity,h.velocity*=h.decay,h.flat?(h.wobble=0,h.wobbleX=h.x+10*h.scalar,h.wobbleY=h.y+10*h.scalar,h.tiltSin=0,h.tiltCos=0,h.random=1):(h.wobble+=h.wobbleSpeed,h.wobbleX=h.x+10*h.scalar*Math.cos(h.wobble),h.wobbleY=h.y+10*h.scalar*Math.sin(h.wobble),h.tiltAngle+=.1,h.tiltSin=Math.sin(h.tiltAngle),h.tiltCos=Math.cos(h.tiltAngle),h.random=Math.random()+2);var y=h.tick++/h.totalTicks,E=h.x+h.random*h.tiltCos,w=h.y+h.random*h.tiltSin,I=h.wobbleX+h.random*h.tiltCos,R=h.wobbleY+h.random*h.tiltSin;if(_.fillStyle="rgba("+h.color.r+", "+h.color.g+", "+h.color.b+", "+(1-y)+")",_.beginPath(),o&&h.shape.type==="path"&&typeof h.shape.path=="string"&&Array.isArray(h.shape.matrix))_.fill(wo(h.shape.path,h.shape.matrix,h.x,h.y,Math.abs(I-E)*.1,Math.abs(R-w)*.1,Math.PI/10*h.wobble));else if(h.shape.type==="bitmap"){var C=Math.PI/10*h.wobble,A=Math.abs(I-E)*.1,D=Math.abs(R-w)*.1,W=h.shape.bitmap.width*h.scalar,F=h.shape.bitmap.height*h.scalar,U=new DOMMatrix([Math.cos(C)*A,Math.sin(C)*A,-Math.sin(C)*D,Math.cos(C)*D,h.x,h.y]);U.multiplySelf(new DOMMatrix(h.shape.matrix));var q=_.createPattern(f.transform(h.shape.bitmap),"no-repeat");q.setTransform(U),_.globalAlpha=1-y,_.fillStyle=q,_.fillRect(h.x-W/2,h.y-F/2,W,F),_.globalAlpha=1}else if(h.shape==="circle")_.ellipse?_.ellipse(h.x,h.y,Math.abs(I-E)*h.ovalScalar,Math.abs(R-w)*h.ovalScalar,Math.PI/10*h.wobble,0,2*Math.PI):vo(_,h.x,h.y,Math.abs(I-E)*h.ovalScalar,Math.abs(R-w)*h.ovalScalar,Math.PI/10*h.wobble,0,2*Math.PI);else if(h.shape==="star")for(var x=Math.PI/2*3,X=4*h.scalar,ae=8*h.scalar,le=h.x,pe=h.y,xe=5,fe=Math.PI/xe;xe--;)le=h.x+Math.cos(x)*ae,pe=h.y+Math.sin(x)*ae,_.lineTo(le,pe),x+=fe,le=h.x+Math.cos(x)*X,pe=h.y+Math.sin(x)*X,_.lineTo(le,pe),x+=fe;else _.moveTo(Math.floor(h.x),Math.floor(h.y)),_.lineTo(Math.floor(h.wobbleX),Math.floor(w)),_.lineTo(Math.floor(I),Math.floor(R)),_.lineTo(Math.floor(E),Math.floor(h.wobbleY));return _.closePath(),_.fill(),h.tick<h.totalTicks}function bo(_,h,y,E,w){var I=h.slice(),R=_.getContext("2d"),C,A,D=c(function(W){function F(){C=A=null,R.clearRect(0,0,E.width,E.height),f.clear(),w(),W()}function U(){i&&!(E.width===s.width&&E.height===s.height)&&(E.width=_.width=s.width,E.height=_.height=s.height),!E.width&&!E.height&&(y(_),E.width=_.width,E.height=_.height),R.clearRect(0,0,E.width,E.height),I=I.filter(function(q){return Eo(R,q)}),I.length?C=d.frame(U):F()}C=d.frame(U),A=F});return{addFettis:function(W){return I=I.concat(W),D},canvas:_,promise:D,reset:function(){C&&d.cancel(C),A&&A()}}}function Ri(_,h){var y=!_,E=!!b(h||{},"resize"),w=!1,I=b(h,"disableForReducedMotion",Boolean),R=r&&!!b(h||{},"useWorker"),C=R?u():null,A=y?mo:go,D=_&&C?!!_.__confetti_initialized:!1,W=typeof matchMedia=="function"&&matchMedia("(prefers-reduced-motion)").matches,F;function U(x,X,ae){for(var le=b(x,"particleCount",se),pe=b(x,"angle",Number),xe=b(x,"spread",Number),fe=b(x,"startVelocity",Number),So=b(x,"decay",Number),No=b(x,"gravity",Number),Ro=b(x,"drift",Number),xi=b(x,"colors",tt),Ao=b(x,"ticks",Number),Mi=b(x,"shapes"),xo=b(x,"scalar"),Mo=!!b(x,"flat"),ki=po(x),Pi=le,gn=[],ko=_.width*ki.x,Po=_.height*ki.y;Pi--;)gn.push(Co({x:ko,y:Po,angle:pe,spread:xe,startVelocity:fe,color:xi[Pi%xi.length],shape:Mi[re(0,Mi.length)],ticks:Ao,decay:So,gravity:No,drift:Ro,scalar:xo,flat:Mo}));return F?F.addFettis(gn):(F=bo(_,gn,A,X,ae),F.promise)}function q(x){var X=I||b(x,"disableForReducedMotion",Boolean),ae=b(x,"zIndex",Number);if(X&&W)return c(function(fe){fe()});y&&F?_=F.canvas:y&&!_&&(_=yo(ae),document.body.appendChild(_)),E&&!D&&A(_);var le={width:_.width,height:_.height};C&&!D&&C.init(_),D=!0,C&&(_.__confetti_initialized=!0);function pe(){if(C){var fe={getBoundingClientRect:function(){if(!y)return _.getBoundingClientRect()}};A(fe),C.postMessage({resize:{width:fe.width,height:fe.height}});return}le.width=le.height=null}function xe(){F=null,E&&(w=!1,e.removeEventListener("resize",pe)),y&&_&&(document.body.contains(_)&&document.body.removeChild(_),_=null,D=!1)}return E&&!w&&(w=!0,e.addEventListener("resize",pe,!1)),C?C.fire(x,le,xe):U(x,le,xe)}return q.reset=function(){C&&C.reset(),F&&F.reset()},q}var mn;function Ai(){return mn||(mn=Ri(null,{useWorker:!0,resize:!0})),mn}function wo(_,h,y,E,w,I,R){var C=new Path2D(_),A=new Path2D;A.addPath(C,new DOMMatrix(h));var D=new Path2D;return D.addPath(A,new DOMMatrix([Math.cos(R)*w,Math.sin(R)*w,-Math.sin(R)*I,Math.cos(R)*I,y,E])),D}function Io(_){if(!o)throw new Error("path confetti are not supported in this browser");var h,y;typeof _=="string"?h=_:(h=_.path,y=_.matrix);var E=new Path2D(h),w=document.createElement("canvas"),I=w.getContext("2d");if(!y){for(var R=1e3,C=R,A=R,D=0,W=0,F,U,q=0;q<R;q+=2)for(var x=0;x<R;x+=2)I.isPointInPath(E,q,x,"nonzero")&&(C=Math.min(C,q),A=Math.min(A,x),D=Math.max(D,q),W=Math.max(W,x));F=D-C,U=W-A;var X=10,ae=Math.min(X/F,X/U);y=[ae,0,0,ae,-Math.round(F/2+C)*ae,-Math.round(U/2+A)*ae]}return{type:"path",path:h,matrix:y}}function To(_){var h,y=1,E="#000000",w='"Apple Color Emoji", "Segoe UI Emoji", "Segoe UI Symbol", "Noto Color Emoji", "EmojiOne Color", "Android Emoji", "Twemoji Mozilla", "system emoji", sans-serif';typeof _=="string"?h=_:(h=_.text,y="scalar"in _?_.scalar:y,w="fontFamily"in _?_.fontFamily:w,E="color"in _?_.color:E);var I=10*y,R=""+I+"px "+w,C=new OffscreenCanvas(I,I),A=C.getContext("2d");A.font=R;var D=A.measureText(h),W=Math.ceil(D.actualBoundingBoxRight+D.actualBoundingBoxLeft),F=Math.ceil(D.actualBoundingBoxAscent+D.actualBoundingBoxDescent),U=2,q=D.actualBoundingBoxLeft+U,x=D.actualBoundingBoxAscent+U;W+=U+U,F+=U+U,C=new OffscreenCanvas(W,F),A=C.getContext("2d"),A.font=R,A.fillStyle=E,A.fillText(h,q,x);var X=1/y;return{type:"bitmap",bitmap:C.transferToImageBitmap(),matrix:[X,0,0,X,-W*X/2,-F*X/2]}}t.exports=function(){return Ai().apply(this,arguments)},t.exports.reset=function(){Ai().reset()},t.exports.create=Ri,t.exports.shapeFromPath=Io,t.exports.shapeFromText=To})(function(){return typeof window<"u"?window:typeof self<"u"?self:this||{}}(),Si,!1);const wt=Si.exports;Si.exports.create;let ie=0;const Mu={apiKey:"AIzaSyCDKY-SQ4mOtNUMp124NgSUiZnV6vglW4A",authDomain:"solvesync-6f45a.firebaseapp.com",databaseURL:"https://solvesync-6f45a-default-rtdb.firebaseio.com",projectId:"solvesync-6f45a",storageBucket:"solvesync-6f45a.appspot.com",messagingSenderId:"...",appId:"..."},ku=js(Mu),ne=Ru(ku),Ae=document.getElementById("answer-input"),K=document.getElementById("student-work"),Ns=document.getElementById("clear-btn"),Pu=document.getElementById("connection-status"),Du=document.getElementById("status-dot"),Rs=document.getElementById("math-problem"),nn=document.getElementById("submit-btn"),As=document.getElementById("hint-btn"),Dt=document.getElementById("chalkboard"),Ot=document.getElementById("streak-num"),xs=document.getElementById("next-btn"),Ms=document.getElementById("high-score-num");let ks=0;Mt(te(ne,".info/connected"),n=>{n.val()===!0&&(Pu.innerText="CLOUD_SYNC_ACTIVE",Du.style.background="#22c55e")});Mt(te(ne,"activeProblem"),n=>{const e=n.val();Rs&&(Rs.innerText=e||"Select a category below to start!")});Mt(te(ne,"worldRecord"),n=>{const e=n.val()||0;Ms&&(Ms.innerText=e)});Ae&&K&&(Ae.addEventListener("input",n=>{ze(te(ne,"currentWork"),n.target.value)}),Mt(te(ne,"currentWork"),n=>{K.innerText=n.val()||"READY_FOR_INPUT"}));Ns&&Ns.addEventListener("click",()=>{ze(te(ne,"currentWork"),""),Ae.value=""});let Ge="add";document.querySelectorAll(".cat-btn").forEach(n=>{n.addEventListener("click",e=>{const t=e.target.getAttribute("data-type");t&&(Ge=t,Ni(Ge))})});xs&&xs.addEventListener("click",()=>{Ge||(Ge="add"),Ni(Ge),K&&(K.innerText="")});function Ni(n){let e=Math.floor(Math.random()*20)+1,t=Math.floor(Math.random()*10)+1,i="",s=0;if(n==="add")i=`${e} + ${t} = ?`,s=e+t;else if(n==="sub")i=`${e+t} - ${t} = ?`,s=e;else if(n==="mult")i=`${e} × ${t} = ?`,s=e*t;else if(n==="div"){let r=Math.floor(Math.random()*50)+10,o=Math.floor(Math.random()*20)+2;i=`${r*o} ÷ ${o} = ?`,s=r}else if(n==="alg"){let r=Math.floor(Math.random()*15)+2,o=Math.floor(Math.random()*3);if(o===0){let a=Math.floor(Math.random()*5)+2,l=Math.floor(Math.random()*10)+1,c=a*(r+l);i=`Solve for x: ${a}(x + ${l}) = ${c}`,s=r}else if(o===1){let a=Math.floor(Math.random()*4)+1,l=a+(Math.floor(Math.random()*5)+2),c=Math.floor(Math.random()*20)+1,f=l*r+c-a*r;i=`Solve for x: ${l}x + ${c} = ${a}x + ${f}`,s=r}else{let a=Math.floor(Math.random()*8)+3,l=Math.floor(Math.random()*6)+2,c=a*r/l;for(;c%1!==0;)r++,c=a*r/l;i=`Solve for x: (${a}x) / ${l} = ${c}`,s=r}}ze(te(ne,"activeProblem"),i),ze(te(ne,"correctAnswer"),s),ze(te(ne,"currentWork"),""),Ae&&(Ae.value="")}function Ps(n){const e=document.getElementById("math-bot"),t=document.getElementById("bot-bubble"),i=document.getElementById("cosmo-rocket"),s=document.getElementById("cosmo-bubble"),r=document.getElementById("chalkboard");if(n%5===0){if(!i||!s)return;i.style.transition="bottom 1s ease-out",i.style.bottom="120px",s.style.display="block",r&&r.classList.add("shake-screen"),setTimeout(()=>{wt({particleCount:60,spread:80,origin:{x:.5,y:.8},colors:["#4b5563","#ff4500"]}),setTimeout(()=>{i.style.transition="bottom 5s cubic-bezier(0.1, 0, 1, 1)",i.style.bottom="180%";const o=Date.now()+4500;(function a(){wt({particleCount:12,angle:270,spread:45,origin:{x:.5,y:.5},colors:["#ff4500","#ffa500","#ffff00"],gravity:2}),Date.now()<o&&requestAnimationFrame(a)})()},1500)},500),setTimeout(()=>{s.style.display="none",r&&r.classList.remove("shake-screen"),i.style.transition="none",i.style.bottom="-400px"},7500)}else if(n%3===0){if(!e||!t)return;e.style.bottom="80px",e.classList.add("bot-dancing"),t.style.display="block",setTimeout(()=>{e.style.bottom="-450px",t.style.display="none",e.classList.remove("bot-dancing")},4500)}}function Ou(){const n=document.getElementById("persistence-banner");n&&setTimeout(()=>{n.style.top="50px",n.style.opacity="1";const t=Date.now()+4e3;(function i(){wt({particleCount:3,angle:60,spread:55,origin:{x:0,y:.3},colors:["#f0abfc","#fbbf24","#ffffff"]}),wt({particleCount:3,angle:120,spread:55,origin:{x:1,y:.3},colors:["#f0abfc","#fbbf24","#ffffff"]}),Date.now()<t&&requestAnimationFrame(i)})(),setTimeout(()=>{n.style.top="20px",n.style.opacity="0",n.style.filter="blur(10px)",setTimeout(()=>{n.style.top="-100px",n.style.filter="blur(0px)"},1e3)},4e3)},8500)}nn&&nn.addEventListener("click",()=>{const n=Ae.value.trim(),e=Number(n);Mt(te(ne,"correctAnswer"),t=>{const i=Number(t.val());e===i&&n!==""?(ie+=1,Ot&&(Ot.innerText=ie),(ie>0&&ie%5===0||ie===3)&&Ps(ie),fo(tn(te(ne),"worldRecord")).then(s=>{const r=s.val()||0;ie>r&&(ze(te(ne,"worldRecord"),ie),console.log("🏆 NEW WORLD RECORD!"))}),wt({particleCount:150,spread:70,origin:{y:.6},colors:["#22d3ee","#f8fafc","#334155"]}),Dt.classList.add("success-glow"),K.style.color="var(--accent-cyan)",ie>=3&&(Dt.style.boxShadow="0 0 50px var(--accent-cyan)"),setTimeout(()=>{alert(`CORRECT! 🚀 STREAK: ${ie}`),Dt.classList.remove("success-glow"),Ae.value="",Ni(Ge)},500)):(ie=0,Ot&&(Ot.innerText="0"),Dt.style.boxShadow="none",K.style.color="var(--danger-red)",alert("TRY AGAIN... ❌ STREAK RESET"))},{onlyOnce:!0})});nn&&nn.addEventListener("click",()=>{Ae.value.trim()!==""&&(ks++,ks%10===0&&Ou())});As&&As.addEventListener("click",async n=>{n.stopPropagation(),K.innerText="Analyzing problem and generating steps...";const e=te(ne,"activeProblem");try{const i=(await fo(e)).val();if(console.log("Sending to AI:",i),!i){K.innerText="NEW_PROBLEM_LOADED",K.style.color="var(--chalk-text)",K.innerText="Please select a math problem first!";return}const l=await(await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key=AIzaSyCyKTyxthpJS0MSYYYLFE6_XLECN3jR5RA",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({contents:[{parts:[{text:`Act as a 4th-8th grade math tutor. Solve this problem: "${i}". 
                        ...
                        Do not include any other text or markdown.`}]}]})})).json();if(l.candidates&&l.candidates[0]){const f=l.candidates[0].content.parts[0].text.replace(/```json|```/g,"").trim(),d=JSON.parse(f);K.innerText=`--- HOW TO SOLVE ---

`+d.join(`

`)}else l.error&&l.error.code===429?(K.innerText="The tutor is busy. Please wait 60 seconds and try again!",K.style.color="var(--danger-red)"):K.innerText="Oops! Something went wrong. Try again."}catch(t){console.error("AI/Firebase Error:",t),K.innerText="Oops! I hit a snag. Try clicking hint again."}});
